...
| `..._tramitacao_backfill.sql` | **Backfill único** do fio da Tramitação — só `INSERT` em `andamento`, idempotente (`not exists`). Recupera o histórico pré-feature: publicações vinculadas, publicações arquivadas, criação e conclusão de atividades (texto humano quando há, resumo automático quando não), anotações (`observacao`) e ajustes de prazo (`prazo_historico`). |
